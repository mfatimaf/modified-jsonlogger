...
import org.mule.extension.jsonlogger.api.pojos.ExceptionDetail;
...
public class JsonloggerOperations implements Initialisable {
    ...
    public void logger(@ParameterGroup(name = "Logger") @Expression(value = NOT_SUPPORTED) LoggerProcessor loggerProcessor,
        ...
        */
        ...
        Map<String, HashMap<String,String>> exceptionDetailsMap = new HashMap<>();
        try {
            PropertyUtils.describe(loggerProcessor).forEach((k, v) -> {
                if (disabledFields.stream().anyMatch(k::equals)) {
                    ...
                    if(v instanceof ExceptionDetail) {
                        try {
                            HashMap<String, String> exceptionDetail = new HashMap<String, String>();
                            ExceptionDetail exceptionObj = (ExceptionDetail) v;
                            exceptionDetail.put("description", exceptionObj.getDescription());
                            exceptionDetail.put("message", exceptionObj.getMessage());
                            exceptionDetail.put("type", exceptionObj.getType());
                            exceptionDetail.put("errorCode", exceptionObj.getErrorCode());
                            exceptionDetail.put("errorCode", exceptionObj.getErrorCode());
                            BeanUtils.setProperty(loggerProcessor, k, null);
                            exceptionDetailsMap.put(k, exceptionDetail);
                        } catch (Exception e) {
                            log.error("Failed parsing field: " + k, e);
                            HashMap<String, String> exceptionDetail = new HashMap<String, String>();
                            exceptionDetail.put("loggerError", "Error parsing expression. See logs for details.");
                            exceptionDetailsMap.put(k, exceptionDetail);
        ...
        if(exceptionDetailsMap.isEmpty() == false){
            JsonNode exceptionDetailNode = om.valueToTree(exceptionDetailsMap);
            mergedLogger.setAll((ObjectNode) exceptionDetailNode);
        }
